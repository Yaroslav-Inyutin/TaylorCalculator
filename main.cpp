...
int main() {
    ...
    while (true) {
        ...
        try {
            ...
            cout << "Выберите режим работы.\nОтправьте R, если хотите найти точность в зависимости от порядка.\nОтправьте N, если хотите найти минимальный порядок разложения для заданной точности. "; // это должно быть в графическом интерфейсе
            char a;
            cin >> a;
            if(a!='R' && a!='N') throw std::invalid_argument("Такого режима нет"); // инвалид аргумент - временное решение, потом займусь обработкой исключений вплотную
